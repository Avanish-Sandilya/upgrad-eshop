            ...
            {pages.map((element) =>
              element.visible ? (
                <Link key={"main_link_" + element.id} to={element.url} style={{ textDecoration: "none" }}>
                  <Button key={"button_link_" + element.id} sx={{ my: 2, color: "white", display: "block", textTransform: "none" }}>
                    <u>{element.label}</u>
                  </Button>
                </Link>
              ) : null
            )}
            {loggedInUser != null && <Logout sx={{ my: 2, ml: 2, color: "white", display: "block" }} />}
          </Box>
        </Toolbar>
      </Container>
    </AppBar>
